...
std::vector<std::vector<std::tuple<int, int>>> get_time_coords(int hour, int minute) {
    std::vector<std::vector<std::tuple<int, int>>> phrase;

    minute = 5 * std::round(minute / 5.0);  // make minute a multiple of 5 (wordclock precision)

    if (minute == 0) {  // for cases like 24 00, which would not be caught below
        if (hour > 23) {hour = 0;}
    }
    ...
    if (minute >= 60) {
        minute = 0;
        if (hour >= 23) { // in cases where 24 (or greater) was passed in as cli arg.
            hour = 0;
        }
    ...
    if (hour == 0) {
        ...
        if (minute == 0) {return phrase;}
    } else { // if not one of the special phrases above, get the hour coords
